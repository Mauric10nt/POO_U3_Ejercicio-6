...
class Menu:
    ...
    def opcion1(self, unaColeccionAparatos: IColeccion):
        ...
        posicion = self.__ingresador.inputInt("Ingrese la posicion en la que desea insertar el aparato: ", "La posicion ingresada no es valida, reintente: ")
        while not insertado:
            try:
                unaColeccionAparatos.insertarElemento(unAparato, posicion)
            except IndexError:
                posicion = self.__ingresador.inputInt("La posicion ingresada no es valida, reintente: ", "La posicion ingresada no es valida, reintente: ")
            else:
                insertado = True
    ...
    def opcion3(self, unaColeccionAparatos: IColeccion):
        posicion = self.__ingresador.inputInt("Ingrese la posicion del aparato que desea mostrar: ", "La posicion ingresada no es valida, reintente: ")
        mostrado = False
        while not mostrado:
            try:
                unaColeccionAparatos.mostrarElemento(posicion)
            except IndexError:
                posicion = self.__ingresador.inputInt("La posicion ingresada no es valida, reintente: ", "[ERROR] La posicion ingresada no es valida, reintente: ")
            else:
                mostrado = True
